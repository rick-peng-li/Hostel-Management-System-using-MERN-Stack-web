...
//         const { hostel, attendancePercentage, totalClasses, attendedClasses } = payload[0].payload;

//         return (
//             <CustomTooltip>
//                 <TooltipText>{hostel}</TooltipText>
...
const CustomTooltipContent = ({ active, payload, dataKey }) => {
    if (active && payload && payload.length) {
        const { hostel, attendancePercentage, totalClasses, attendedClasses, marksObtained, subName } = payload[0].payload;

        return (
            <CustomTooltip>
                {dataKey === "attendancePercentage" ? (
                    <>
                        <TooltipMain>{hostel}</TooltipMain>
...
const CustomBarChart = ({ chartData, dataKey }) => {
    const hostels = chartData.map((data) => data.hostel);
    const distinctColors = generateDistinctColors(hostels.length);

    return (
        <BarChart width={500} height={500} data={chartData}>
            <XAxis dataKey={dataKey === "marksObtained" ? "subName.subName" : "hostel"} />
            <YAxis domain={[0, 100]} />
